...
@section('content')
    ...
    <!-- <a href="{{ route('campaigns.create') }}" class="mt-20 px-6 py-2 bg-yellow-400 text-blue-900 font-semibold rounded-lg shadow-md hover:bg-yellow-500 transition">
            Start a Campaign
        </a> -->
</div>
    ...
    @if(isset($campaigns) && count($campaigns) > 0)
        @foreach($campaigns as $campaign)
            ...
            <img src="{{ $campaign->image ? asset('storage/' . $campaign->image) : 'default-image-path.jpg' }}" alt="{{ $campaign->title }}" class="w-full h-48 object-cover">
            <div class="p-4">
                    ...
                    <div class="flex justify-between items-center mt-4">
                        ...
                        <a href="{{ route('campaigns.show', $campaign->id) }}" class="text-blue-500 hover:underline">View</a>
                    </div>
                </div>
            </div>
        @endforeach
